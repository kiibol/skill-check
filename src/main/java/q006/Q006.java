...
public class Q006 {
    ...
    public static void main(String[] args) {
        ...
        String value = "";
        try {
            System.out.println("逆ポーランド記法を入力してください");
            value += s.nextLine();
        } finally {
            s.close();
        }

        List<IValue> iValues = parseLine(value);
        Stack<BigDecimal> stackValue = new Stack<>();
        iValues.forEach(item -> item.execute(stackValue));
